...
export async function scheduleMedicationNotifications(reminder: any) {
  ...
  for (const [i, timeStr] of reminder.alarms.entries()) {
    const label = reminder.alarm_labels?.[i];
    ...
    await Notifications.scheduleNotificationAsync({
      content: {
        title: i18next.t('notifications.doseDueTitle'),
        body: label
          ? i18next.t('notifications.doseDueBodyWithLabel', { label, dosage: reminder.selected_dosage, medName: reminder.med_name })
          : i18next.t('notifications.doseDueBody', { dosage: reminder.selected_dosage, medName: reminder.med_name }),
        data: {
...
export async function rescheduleNextOccurrence(data: any) {
  ...
  await Notifications.scheduleNotificationAsync({
    content: {
      title: i18next.t('notifications.doseDueTitle'),
      body: data.label
        ? i18next.t('notifications.doseDueBodyWithLabel', { label: data.label, dosage: data.dosage, medName: data.medName })
        : i18next.t('notifications.doseDueBody', { dosage: data.dosage, medName: data.medName }),
      data,
      priority: Notifications.AndroidNotificationPriority.MAX,
      sound: Platform.OS === 'ios' ? true : 'alarm.wav',
    },
